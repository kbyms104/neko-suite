# PowerShell Release Build Script for Bongo Format
$ErrorActionPreference = "Stop"

Write-Host "===================================================" -ForegroundColor Cyan
Write-Host "🐱 Bongo Format 릴리즈 배포 빌드를 시작합니다..." -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan

$AppDir = Join-Path $PSScriptRoot "..\apps\bongo-format"
Set-Location $AppDir
...
$ReleaseExe = Join-Path $AppDir "src-tauri\target\release\bongo-format.exe"
...
if (Test-Path $ReleaseExe) {
    Copy-Item $ReleaseExe (Join-Path $PortableDir "Bongo Format.exe") -Force
    Copy-Item $ReleaseExe (Join-Path $WebDownloadsDir "Bongo Format.exe") -Force
}

$BundleDir = Join-Path $AppDir "src-tauri\target\release\bundle"
Write-Host "`n===================================================" -ForegroundColor Green
Write-Host "🎉 Bongo Format 릴리즈 배포 패키징이 완료되었습니다!" -ForegroundColor Green
Write-Host "포터블 파일: portable\Bongo Format.exe" -ForegroundColor Green
